...
#   * `AIE_LLVM_Toolchain_jll` ships the AIE LLVM fork (`clang`/`lld`/`llc`/`opt`) that `aiecc`
...
generation and linking; it defaults to `AIE_LLVM_Toolchain_jll`. Override it to point at
another llvm-aie (e.g. one whose AIE bare-metal runtimes -- `libclang_rt.builtins.a`,
`crt0.o`/`crt1.o`, `libc.a`/`libm.a` -- are present, which `AIE_LLVM_Toolchain_jll` does not
yet ship).
...
function aiecc_compile(
    ...
    # aiecc and its xclbinutil subprocess are chatty on stdout/stderr. Swallow it by
    # default but capture it so a failure can still be diagnosed; `verbose` (which also
    # passes `--verbose` to aiecc) streams it straight through.
    if verbose
        run(cmd)
    else
        io = IOBuffer()
